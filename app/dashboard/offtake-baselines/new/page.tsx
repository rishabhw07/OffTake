...
export default function NewOfftakeBaselinePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/offtake-baselines', {
        method: 'POST',
      ...
      if (response.ok) {
        router.push('/dashboard/offtake-baselines')
      } else {
        const data = await response.json()
        alert(data.error || 'Failed to create offtake baseline')
      }
    } catch (error) {
  ...
  return (
    <DashboardLayout>
      <div className="max-w-4xl">
        <h1 className="text-3xl font-bold mb-6">Create Offtake Baseline</h1>
